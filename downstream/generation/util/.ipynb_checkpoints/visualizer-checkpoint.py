...
class Visualizer():
                ...
                if isinstance(image_numpy, list):
                    ...
                    if label == 'synthesized_image':
                        ...
                        for i in range(len(visuals)):
                            img_path = os.path.join(self.img_dir, 'epoch%.3d_iter%.3d_%s_%d.png' % (epoch, step, label, i+1))
                            util.save_image(image_numpy[:,:,i], img_path)
                    elif label == 'real_image':
                        image_numpy = image_numpy.transpose(1, 2, 0)
                        for i in range(len(visuals)):
                            img_path = os.path.join(self.img_dir, 'epoch%.3d_iter%.3d_%s_%d.png' % (epoch, step, label, i+1))
                            util.save_image(image_numpy[:,:,i], img_path)
                    elif label == 'input_label':
                        for i in range(len(visuals)):
                            img_path = os.path.join(self.img_dir, 'epoch%.3d_iter%.3d_%s_%d.png' % (epoch, step, label, i+1))
                            util.save_image(image_numpy, img_path)
    ...
    def save_images(self, webpage, visuals, image_path, num):        
        visuals = self.convert_visuals_to_numpy(visuals)        
        ...
        for label, image_numpy in visuals.items():
            ...
            image_name = os.path.join(label, '%d.png' % (num))
