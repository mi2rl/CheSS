...
from models.networks.resnet_3ch import resnet50 as _3ch_resnet50
from models.networks.resnet_1ch import resnet50 as _1ch_resnet50
...
# Perceptual loss that uses a pretrained Resnet network
class ResnetLoss(nn.Module):
    def __init__(self, gpu_ids, perceptual_channel):
        super(ResnetLoss, self).__init__()
        resnet_model =  _1ch_resnet50() if perceptual_channel == 1 else _3ch_resnet50()
        if perceptual_channel == 3:
            checkpoint = torch.load('pretrained_model/resnet50-19c8e357.pth')
            resnet_model.load_state_dict(checkpoint , strict=False)
        elif perceptual_channel == 1:
            load_moco_model(resnet_model, 'pretrained_model/SSAP_temp.pth.tar')
            
        self.resnet = resnet_model.cuda()
        self.criterion = nn.L1Loss()
        self.weights = [1.0 / 32, 1.0 / 16, 1.0 / 8, 1.0 / 4, 1.0]
        self.perceptual_channel = perceptual_channel

    def forward(self, x, y):
        if self.perceptual_channel == 3:
            x = torch.cat([x, x, x], dim=1)
            y = torch.cat([y, y, y], dim=1)
