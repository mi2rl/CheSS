...
def _resnet(arch, block, layers, perceptual_channel, pretrained_weight_path, pretrained, progress, **kwargs):
    if perceptual_channel == 3:
        ...
        model = ResNet(block, layers, **kwargs)
        load_moco_model(model, pretrained_weight_path)
    return model

def ResNet50(perceptual_channel, pretrained_weight_path, pretrained=False, progress=True, **kwargs):
    ...
    """
    return _resnet('resnet50', Bottleneck, [3, 4, 6, 3], perceptual_channel, pretrained_weight_path, pretrained, progress, 
                    **kwargs)
